  ...
  return (
    ...
    >
      <nav className="page-wrap flex flex-wrap items-center gap-x-3 gap-y-2 py-3 sm:py-4">
        <h2 className="m-0 flex-shrink-0">
          ...
          >
            <div className="nb-logo-mark relative flex h-10 w-10 items-center justify-center rounded-lg border-2 border-[var(--nb-border-color,#0f0f0f)] bg-[var(--nb-yellow,#ffe566)] shadow-[3px_3px_0px_var(--nb-border-color,#0f0f0f)] transition-all duration-150 hover:shadow-none hover:translate-x-[3px] hover:translate-y-[3px]">
              <span className="text-xl font-black text-[var(--nb-on-accent,#111)]">fx</span>
        ...
        >
          <span className="text-lg font-black">{mobileMenuOpen ? "×" : "☰"}</span>
        </button>

        <div className={`order-3 ${mobileMenuOpen ? "flex" : "hidden"} sm:flex w-full sm:w-auto flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-x-4 gap-y-1 pb-2 sm:pb-0 sm:ml-4`}>
          {navItems.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              ...
              activeProps={{
                className: "nb-nav-link-active border-[var(--nb-border-color,#0f0f0f)] bg-[var(--nb-teal,#7ee8e2)] shadow-[3px_3px_0px_var(--nb-border-color,#0f0f0f)]",
              }}
